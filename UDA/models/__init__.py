...
def create_model(cfg, cfg_model, writer, logger):
    ...
    """
    # model = find_model_using_name(cfg['name'])
    # model = CustomModel(cfg, writer, logger)
    # instance = model(cfg)
    # print("model [%s] was created" % type(instance).__name__)
    model = find_model_using_name(cfg_model['name'])
    model = model(cfg, cfg_model, writer, logger)
    return model
