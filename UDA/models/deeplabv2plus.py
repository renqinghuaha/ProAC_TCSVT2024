...
class Classifier_Module2(nn.Module):
    def __init__(self, inplanes, dilation_series, padding_series, num_classes, droprate = 0.1, use_se = True):
        ...
        self.proj = nn.Conv2d(256, 256, kernel_size=1, padding=0, dilation=1, bias=False)
        ##########init#######
        for m in self.conv2d_list:
    ...
    def forward(self, x, get_feat=False):
        ...
        if get_feat:
            out_dict = {}
            out_dict['feat'] = self.proj(out)
            out_dict['out'] = self.head(out)
            return out_dict
        else:
            out = self.head(out)
            return out
...
def Deeplabplus(BatchNorm, num_classes=21, freeze_bn=False, restore_from=None, initialization=None, bn_clr=False):
    ...
    if restore_from is not None: 
        ...
        pretrain_dict = checkpoint['ResNet101']["model_state"]
        model_dict = {}
        state_dict = model.state_dict()
        for k, v in pretrain_dict.items():
            if k in state_dict:
