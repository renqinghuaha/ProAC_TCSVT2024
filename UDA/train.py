...
def train(cfg, writer, logger):
    ...
    for epoch in range(epoches):
        ...
        for target_batch in datasets.target_train_loader:
            ...
            loss_GTA, loss_CTS, loss_feat = model.step_contrast(source_image, source_label, target_image, target_label)
            time_meter.update(time.time() - start_ts)
            if (i + 1) % cfg['training']['print_interval'] == 0:
                fmt_str = "Epoches [{:d}/{:d}] Iter [{:d}/{:d}]  loss_GTA: {:.4f}  loss_CTS: {:.4f}  loss_feat: {:.4f}  Time/Image: {:.4f}"
                print_str = fmt_str.format(epoch + 1, epoches, i + 1, cfg['training']['train_iters'],
                                           loss_GTA,
                                           loss_CTS,
                                           loss_feat,
